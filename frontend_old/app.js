  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "你好！我是厦小嘉，厦门大学嘉庚学院图书馆的专属智能助手。有什么我可以帮你的吗？",
    },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          // Only send history that the backend expects
      ...
      setIsLoading(false); // Stop the main loading indicator since stream started
  ...
  return (
    <div className="tab-content">
      <div className="kb-container">
        ...
        <button id="save-kb-button" onClick={handleSave} disabled={isSaving}>
          {isSaving ? "保存中..." : "保存到知识库"}
        </button>
      </div>
    </div>
